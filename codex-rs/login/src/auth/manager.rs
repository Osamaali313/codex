...
pub async fn enforce_login_restrictions(config: &AuthConfig) -> std::io::Result<()> {
    ...
    .await?;
    let stored_bedrock_api_key =
        load_stored_bedrock_api_key(&config.codex_home, config.auth_credentials_store_mode)?;
    let auth_mode = auth
        .as_ref()
        .map(CodexAuth::auth_mode)
        .or_else(|| stored_bedrock_api_key.as_ref().map(|_| AuthMode::BedrockApiKey));
...
impl AuthManager {
    ...
    async fn new_with_workspace_restriction(
        ...
        let bedrock_api_key = load_stored_bedrock_api_key(&codex_home, auth_credentials_store_mode)
            .ok()
            .flatten();
    ...
    fn load_bedrock_api_key_from_storage(&self) -> Option<BedrockApiKeyAuth> {
        load_stored_bedrock_api_key(&self.codex_home, self.auth_credentials_store_mode)
            .ok()
            .flatten()
    }
